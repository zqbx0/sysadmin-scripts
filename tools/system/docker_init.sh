...
LIB_DIR="$SCRIPT_DIR/"$(cd "$(dirname "${BASH_SOURCE[0]}")" ../lib../lib cd ../lib ../lib../lib pwd)""
...
# ==================== 引入公共库 ====================
LIB_DIR="$(cd "$(dirname "\${BASH_SOURCE[0]}")" && cd "$(cd "$(dirname "${BASH_SOURCE[0]}")" ../lib../lib cd ../lib ../lib../lib pwd)" && pwd)"
source "\$LIB_DIR/common.sh"
source "\$LIB_DIR/argo.sh" 2>/dev/null
source "\$LIB_DIR/config.sh" 2>/dev/null

# ==================== 脚本特定配置 ====================
WORK_DIR="/etc/$(basename "${SCRIPT%.sh}")"
TEMP_DIR="/tmp/$(basename "${SCRIPT%.sh}")"
LOG_FILE="\$WORK_DIR/$(basename "${SCRIPT%.sh}").log"
